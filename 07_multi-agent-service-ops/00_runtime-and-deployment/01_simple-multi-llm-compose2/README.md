...
`backend/`, `frontend/`, `compose.yml`, 또는 각 폴더의 `.env`를 수정해도 이미 실행 중인
Container는 자동으로 바뀌지 않습니다. 예를 들어 `GEMMA_MODEL=gemma4:e2b`로 바꾼 뒤에는
...
`backend/.env`에 OpenAI 또는 Gemini Key와 Provider 설정을 입력합니다.
...
`backend/.env`의 주소는 Host에서 직접 실행할 때의 값입니다. Compose는
Backend Container에 `host.docker.internal` 주소를 덮어씁니다.
...
운영자가 전달한 `compose.release.yml`과 세 곳의 `.env.example`을 폴더 구조에 맞게
둡니다. 각 `.env`를 만들고 최상위 `.env`의 Image 주소를 실제 Docker Hub
사용자명과 버전으로 바꿉니다.
